...
public class SendEmailUtil {
    ...
    public static String myEmailAccount = "cxy263462@sina.com";
    public static String myEmailPassword = "LOVEmeina521!@";
    // 发件人邮箱的 SMTP 服务器地址, 必须准确, 不同邮件服务器地址不同, 一般(只是一般, 绝非绝对)格式为: smtp.xxx.com
    // 网易163邮箱的 SMTP 服务器地址为: smtp.163.com
    public static String myEmailSMTPHost = "smtp.sina.com";
    // 收件人邮箱（替换为自己知道的有效邮箱）
    public static String receiveMailAccount = "1051217793@qq.com";
    public static void main(String[] args) {
          send("哈哈哈，你猜猜我是谁", "福利来啦！！！");
    }
    /**
     * @Description 发送
     * @author ChenXianyong
     ...
     * @param title   邮件主题
     */
    public static void send(String context, String title) {
          try {
               ...
               // 3. 创建一封邮件
               MimeMessage message = createMimeMessage(session, myEmailAccount, receiveMailAccount, context, title);
     ...
     * @throws Exception
     */
    public static MimeMessage createMimeMessage(Session session, String sendMail, String receiveMail, String context,
               String title) throws Exception {
          // 1. 创建一封邮件
          MimeMessage message = new MimeMessage(session);
          // 2. From: 发件人（昵称有广告嫌疑，避免被邮件服务器误认为是滥发广告以至返回失败，请修改昵称）
          message.setFrom(new InternetAddress(sendMail, "OKE家", "UTF-8"));
          // 3. To: 收件人（可以增加多个收件人、抄送、密送）
          message.setRecipient(MimeMessage.RecipientType.TO, new InternetAddress(receiveMail, "开发人员", "UTF-8"));
          // Cc: 抄送（可选）
//             message.setRecipient(MimeMessage.RecipientType.CC,
//                       new InternetAddress("lizhizhi@gdcattsoft.com", "USER_EE", "UTF-8"));
          // Cc: 抄送（可选）
//             message.setRecipient(MimeMessage.RecipientType.CC,
//                       new InternetAddress("liangzhe@gdcattsoft.com", "USER_EE", "UTF-8"));
     ...
     * @param title   邮件主题
     */
    public static void createEmail(String title, String context) {
          try {
               // 1. 创建一封邮件
               ...
               // 真正要发送时, 邮箱必须是真实有效的邮箱。
               message.setFrom(new InternetAddress(myEmailAccount, "测试服务器", "UTF-8"));
               // 3. To: 收件人
               message.setRecipient(MimeMessage.RecipientType.TO, new InternetAddress(receiveMailAccount, "测试", "UTF-8"));
               // To: 增加收件人（可选）
